...
const (
	...
	// SummarySample is used to match WAL records of type Summaries.
	SummarySample Type = 11
)
...
func (*Decoder) Type(rec []byte) Type {
	...
	case Series, Samples, Tombstones, Exemplars, MmapMarkers, Metadata, HistogramSamples, FloatHistogramSamples, CustomBucketsHistogramSamples, CustomBucketsFloatHistogramSamples, SummarySample:
		return t
	}
	return Unknown
}
...
func (*Decoder) SummarySamples(rec []byte, summaries []RefSummarySample) ([]RefSummarySample, error) {
	dec := encoding.Decbuf{B: rec}
	t := Type(dec.Byte())
	if t != SummarySample {
		return nil, errors.New("invalid record type")
...
func (*Encoder) SummarySamples(summaries []RefSummarySample, b []byte) []byte {
	buf := encoding.Encbuf{B: b}
	buf.PutByte(byte(SummarySample))
